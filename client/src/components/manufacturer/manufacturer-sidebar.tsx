...
export function ManufacturerSidebar({ user }: { user: AuthUser }) {
  const [, navigate] = useLocation();
  const { toast } = useToast();
  const { logoutMutation } = useAuth();
  const [openMobileMenu, setOpenMobileMenu] = useState(false);
  
  // Base navigation items for manufacturers
  const defaultNavigation = [
    ...
    },
    {
      ...
      href: "/status-update",
      icon: <Hammer className="mr-2" size={16} />,
      id: "status-update",
    },
    {
      name: "Completion & Shipping",
      href: "/shipping",
      icon: <TruckIcon className="mr-2" size={16} />,
      id: "shipping",
    },
    {
      name: "Order History",
    ...
    },
    {
      ...
      href: "/metrics",
      icon: <BarChart className="mr-2" size={16} />,
      id: "metrics",
    },
    {
      name: "Notifications",
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex md:flex-col md:fixed md:inset-y-0 z-10 bg-white border-r w-64">
        <NavContent />
      </div>
